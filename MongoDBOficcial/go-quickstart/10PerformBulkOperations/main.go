...
func main() {
	...
	models := []mongo.WriteModel{
		...
		//^It's useful when you want to replace the entire content of a document with a new set of fields.
		mongo.NewReplaceOneModel().SetFilter(bson.D{{"name", "Cafe Tomato"}}).
			SetReplacement(Restaurant{Name: "Cafe Zucchini", Cuisine: "French"}),

		//^It's useful when you want to modify specific fields of the document without replacing the entire document.
		mongo.NewUpdateOneModel().SetFilter(bson.D{{"name", "Cafe Zucchini"}}).
			SetUpdate(bson.D{{"$set", bson.D{{"name", "Zucchini Land"}}}}),
	}
	opts := options.BulkWrite().SetOrdered(true) //operations will do ordered
